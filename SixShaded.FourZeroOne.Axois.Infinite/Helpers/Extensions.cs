...
{
    public static IOption<HexPos> GetStruct(this IRoveggi<uRelativeCoordinate> coordinate) =>
        coordinate.GetComponent(uRelativeCoordinate.R).Check(out var rc) &&
        coordinate.GetComponent(uRelativeCoordinate.U).Check(out var uc) &&
        coordinate.GetComponent(uRelativeCoordinate.D).Check(out var dc)
            ? new HexPos(rc.Value, uc.Value, dc.Value).AsSome()
            : new None<HexPos>();

    public static IRoveggi<uRelativeCoordinate> GetRoveggi(this HexPos pos) =>
        new Roveggi<uRelativeCoordinate>()
            .WithComponent(uRelativeCoordinate.R, pos.R)
            .WithComponent(uRelativeCoordinate.U, pos.U)
            .WithComponent(uRelativeCoordinate.D, pos.D);
