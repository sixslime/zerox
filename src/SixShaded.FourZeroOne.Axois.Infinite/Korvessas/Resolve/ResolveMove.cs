namespace SixShaded.FourZeroOne.Axois.Infinite.Korvessas.Resolve;

using Rovetus.Constructs.Move;
using Rovetus.Constructs.Resolved;
using ResolvedObj = IRoveggi<u.Constructs.Resolved.uResolvedMove>;
using Core = Core.Syntax.Core;

public record ResolveMove(IKorssa<IRoveggi<uMove>> move) : Korvessa<IRoveggi<uMove>, IMulti<ResolvedObj>>(move)
{
    protected override RecursiveMetaDefinition<IRoveggi<uMove>, IMulti<ResolvedObj>> InternalDefinition() =>
        (_, iMove) =>
            Core.kSubEnvironment<IMulti<ResolvedObj>>(
            new()
            {
                Environment =
                [
                    iMove.kRef()
                        .kGetRovi(uMove.ENVIRONMENT_PREMOD)
                        .kExecute(),
                ],
                Value =
                    Core.kSelector<IMulti<ResolvedObj>>(
                    [
                        // NUMERICAL:
                        () =>
                            iMove.kRef()
                                .kIsType<IRoveggi<uNumericalMove>>()
                                .kResolve(),

                        // POSITIONL:
                        () =>
                            iMove.kRef()
                                .kIsType<IRoveggi<uPositionalMove>>()
                                .kResolve()
                                .kYield(),
                    ]),
            });
}
