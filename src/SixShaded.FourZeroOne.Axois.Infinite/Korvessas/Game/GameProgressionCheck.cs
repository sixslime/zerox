...
public record GameProgressionCheck(IKorssa<ProgramState> since) : Korvessa<ProgramState, Bool>(since)
{
    protected override RecursiveMetaDefinition<ProgramState, Bool> InternalDefinition() =>
        (_, iSince) =>
            Infinite.AllPlayers
                .kAnyMatch(
                iPlayer =>
