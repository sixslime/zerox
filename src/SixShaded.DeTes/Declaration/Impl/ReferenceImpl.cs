...
internal class ReferenceImpl<R> : IReferenceAccessor, IDeTesReference<R>
    where R : class, Rog
{
    ...
    private DeTesInvalidTestException MakeUnevaluatedException() =>
        ...
        {
            Value =
                new EDeTesInvalidTest.ReferenceUsedBeforeEvaluated
                {
    ...
    R IDeTesReference<R>.Roggi => _roggi.Check(out var v) ? v.DeTesUnwrap() : throw MakeUnevaluatedException();
    IOption<R> IDeTesReference<R>.RoggiUnstable => _roggi.Check(out var v) ? v : throw MakeUnevaluatedException();
    IMemoryFZO IDeTesReference<R>.Memory => _memory.Check(out var v) ? v : throw MakeUnevaluatedException();
