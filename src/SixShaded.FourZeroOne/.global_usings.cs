
global using SixShaded.NotRust;
global using SixShaded.SixLib.GFunc;
global using SixShaded.FourZeroOne.Resolution;
global using SixShaded.FourZeroOne.Token;
global using SixShaded.FourZeroOne.Rule;
global using SixShaded.FourZeroOne.Macro;
global using SixShaded.FourZeroOne.Handles;
global using Res = SixShaded.FourZeroOne.Resolution.IResolution;
global using ResOpt = SixShaded.NotRust.IOption<SixShaded.FourZeroOne.Resolution.IResolution>;
global using Tok = SixShaded.FourZeroOne.Token.IToken<SixShaded.FourZeroOne.Resolution.IResolution>;
global using Addr = SixShaded.FourZeroOne.Resolution.IMemoryAddress<SixShaded.FourZeroOne.Resolution.IResolution>;
global using Rul = SixShaded.FourZeroOne.Rule.Unsafe.IRule<SixShaded.FourZeroOne.Resolution.IResolution>;
global using MorseCode.ITask;