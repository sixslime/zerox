    ...
    where ROut : class, Res
{
    ...
    protected sealed override ITask<IOption<ROut>> StandardResolve(ITokenContext runtime, IOption<Res>[] args) => Evaluate(runtime, args[0].RemapAs(x => (RArg1)x));
    ...
    where ROut : class, Res
{
    ...
    protected sealed override ITask<IOption<ROut>> StandardResolve(ITokenContext runtime, IOption<Res>[] args) => Evaluate(runtime, args[0].RemapAs(x => (RArg1)x), args[1].RemapAs(x => (RArg2)x));
    ...
    where ROut : class, Res
{
    ...
    protected sealed override ITask<IOption<ROut>> StandardResolve(ITokenContext runtime, IOption<Res>[] args) => Evaluate(runtime, args[0].RemapAs(x => (RArg1)x), args[1].RemapAs(x => (RArg2)x), args[2].RemapAs(x => (RArg3)x));
