...
{
    public static Tokens.SubEnvironment<R> tSubEnvironment<R>(Structure.Token.SubEnvironment<R> block) where R : class, Res => new(block.Environment, block.Value);

    public static Tokens.Multi.Union<Res> t_Env(params Tok[] environment) => new(environment.Map(x => x.tYield()));
}
