...
namespace SixShaded.FourZeroOne.Core.Tokens.Multi
{
    ...
    public sealed record Count : Token.Defined.Function<IMulti<Res>, Number>
    {
        ...
        protected override ITask<IOption<Number>> Evaluate(ITokenContext _, IOption<IMulti<Res>> in1)
        {
            return new Number() { Value = in1.RemapAs(x => x.Count).Or(0) }.AsSome().ToCompletedITask();
        }
