...
namespace SixShaded.FourZeroOne.Core.Tokens
{
    public sealed record DynamicAssign<R> : Token.Defined.StandardToken<Resolutions.Instructions.Assign<R>> where R : class, Res
    {
        ...
        protected override ITask<IOption<Resolutions.Instructions.Assign<R>>> StandardResolve(ITokenContext runtime, IOption<Res>[] args)
        {
            return args[0].RemapAs(x => new Resolutions.Instructions.Assign<R>() { Address = AssigningAddress, Subject = (R)x }).ToCompletedITask();
        }
