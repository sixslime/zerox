...
public record IfElse<R> : Token.Defined.Function<Bool, MetaFunction<R>, MetaFunction<R>, MetaFunction<R>> where R : class, Res
{
    ...
    protected override ITask<IOption<MetaFunction<R>>> Evaluate(ITokenContext runtime, IOption<Bool> in1, IOption<MetaFunction<R>> in2, IOption<MetaFunction<R>> in3) => in1.RemapAs(x => x.IsTrue ? in2 : in3).Press().ToCompletedITask();
