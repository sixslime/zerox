...
public sealed record Exclusion<R> : Korssa.Defined.Function<IMulti<R>, IMulti<R>, Roggis.Multi<R>> where R : class, Rog
{
    ...
    protected override ITask<IOption<Roggis.Multi<R>>> Evaluate(IKorssaContext _, IOption<IMulti<R>> in1, IOption<IMulti<R>> in2) =>
        in1.RemapAs(from => new Roggis.Multi<R> { Values = in2.RemapAs(sub => from.Elements.Except(sub.Elements)).Or([]).ToPSequence() })
            .ToCompletedITask();
