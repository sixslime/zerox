...
public sealed record GetIndex<R> : Korssa.Defined.Function<IMulti<R>, Number, R>
    where R : class, Rog
{
    ...
    protected override ITask<IOption<R>> Evaluate(IKorssaContext _, IOption<IMulti<R>> in1, IOption<Number> in2) =>
        (in1.Check(out var from) && in2.Check(out var index)
            ? from.At(index.Value - 1)
            : new None<IOption<R>>()).Press().ToCompletedITask();
