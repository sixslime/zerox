...
public sealed record Count : Korssa.Defined.Function<IMulti<Rog>, Number>
{
    ...
    protected override ITask<IOption<Number>> Evaluate(IKorssaContext _, IOption<IMulti<Rog>> in1) => new Number { Value = in1.RemapAs(x => x.Count).Or(0) }.AsSome().ToCompletedITask();
