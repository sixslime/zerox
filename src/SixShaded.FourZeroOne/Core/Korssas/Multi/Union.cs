...
public sealed record Union<R> : Korssa.Defined.PureCombiner<IMulti<R>, Roggis.Multi<R>> where R : class, Rog
{
    ...
    protected override Roggis.Multi<R> EvaluatePure(IEnumerable<IMulti<R>> inputs) => new() { Values = inputs.Map(x => x.Elements).Flatten().ToPSequence() };
