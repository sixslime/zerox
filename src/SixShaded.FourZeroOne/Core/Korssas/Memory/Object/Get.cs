    ...
    where R : class, Rog
{
    ...
    protected override ITask<IOption<R>> Evaluate(IKorssaContext runtime, IOption<IRoveggi<IMemoryRovetu<R>>> in1) => in1.RemapAs(x => runtime.CurrentMemory.GetObject(x)).Press().ToCompletedITask();
