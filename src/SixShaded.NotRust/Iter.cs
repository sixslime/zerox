...
public static class Iter
{
    ...
    public static IEnumerable<TResult> FilterMap<TIn, TResult>(this IEnumerable<TIn> enumerable, Func<TIn, IOption<TResult>> mapFunction)
    {
        foreach (var e in enumerable)
            if (mapFunction(e).Check(out var some))
                yield return some;
    }
    ...
    /// <summary>
    ///     WARNING: generates *infinite* values. Must be used with a terminator (such as
    ///     <see cref="Until{T}(IEnumerable{T}, Predicate{T})" />
    /// </summary>
    /// <returns></returns>
    ...
    public static IEnumerable<int> ToIter(this Range range, bool inclusiveEnd = false) => Range(range.Start.Value, range.End.Value, inclusiveEnd);
